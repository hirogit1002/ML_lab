...
def cv(X, y, method, parameters,nfolds=10, nrepetitions=5,loss_function=zero_one_loss):
    ...
    pr = list(it.product(*[kp,reg]))
    lenpr = len(pr)
    krrset = [method(knl,pr[i][0],pr[i][1]) for i in range(lenpr)]
    tm = time.time()
    print("Start iterarion:  ",time.time()-tm)
    for i in range(nrepetitions):
        ...
        for j in range(nfolds):
            ...
            print("End ",i+1,"te iteration. ",j+1,"te fold.")
            print("Time: ",time.time()-tm)
            print("Remain time: ",(time.time()-otm)*(nfolds-(j))*(nrepetitions-(i)))
            
    output = krrset[np.argmin(losssum)]
    output.cvloss = np.min(losssum)
    if(lenpr==1):        
        D = (losssum -losssum.mean())**2
        output = krrset[np.argmin(D)]
        output.cvloss = losssum[np.argmin(D)]
    return output
